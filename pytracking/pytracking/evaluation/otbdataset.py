...
class OTBDataset(BaseDataset):
    ...
    def __init__(self, path):
        ...
        self.sequence_info_list = self._filter_sequence_info_list()
    ...
    def _filter_sequence_info_list(self):
        available_sequences = []
        # iterate through all the sequences in dictionary and check that the directory with the same name existis
        for sequence_info in self.sequence_info_list:
            name = sequence_info['name']
            if os.path.isdir(os.path.join(self.base_path, name)):
                available_sequences.append(sequence_info)
