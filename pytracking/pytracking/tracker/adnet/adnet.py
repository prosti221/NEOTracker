...
from .models import ADNet, ADNetEnsemble, ADNetEnhanced
...
class ADNetTracker(BaseTracker):
    ...
    def initialize(self, image, info):
        ...
        # Init network
        if self.params.ensemble_mode:
            print("Running in ensemble mode!")
            self.adnet = ADNetEnsemble(
                n_actions=self.params.num_actions, 
                n_action_history=self.params.num_action_history, 
                n_heads=self.params.n_heads
            )
            self.adnet.load_network(
                self.params.model_path, 
                freeze_backbone=True, 
                fc6_checkpoints_path=self.params.heads_checkpoint_path
            )
        elif self.params.use_efficientnet:
            self.adnet = ADNetEnhanced(n_actions=self.params.num_actions, n_action_history=self.params.num_action_history)
    ...
    def update_action_history(self, action_idx):
        ...
        self.action_history_oh[0, n:] = self.action_history_oh[0, :-n].clone()
    ...
    def tracking_procedure(self, image, bbox):
        ...
        while move_counter < opts['num_action_step_max'] and prev_action != opts['stop_action']:
            ...
            #print(f"Action probabilities: {torch.sum(actions)}")
    ...
    def redetect(self, image, bbox, confidence):
        ...
        #if mean_score > confidence and confidence < 0.01:
            #print(f"Mean score {mean_score} > confidence {confidence}")
