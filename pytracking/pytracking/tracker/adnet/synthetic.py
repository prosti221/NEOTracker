...
class DummySequence(VOTSequence):
    ...
    def _generate(base, length, size):
        ...
        for i in range(length):
            #background_color = Image.fromarray(np.random.normal(250, 5, (size[1], size[0], 3)).astype(np.uint8))
            frame_color = background_color.copy()

            # Update position and velocity randomly
            if i % gait_length == 0:
                update_x = np.random.normal(0.7, 1) * np.random.choice([-1, 1])
                update_y = np.random.normal(0.7, 1) * np.random.choice([-1, 1])
            
            x += update_x * speed
            y += update_y * speed

            if x < 0 or x > size[0] - template.size[0]:
                speed *= -1
                x  = np.clip(x, 0, size[0] - template.size[0])
            if y < 0 or y > size[1] - template.size[1]:
                speed *= -1
                y = np.clip(y, 0, size[1] - template.size[1])
            x = int(x); y = int(y)
    ...
    """
    # Write a classic DVD screensaver sequence
    @staticmethod
    def _generate(base, length, size):
        background_color = Image.fromarray(np.random.normal(15, 5, (size[1], size[0], 3)).astype(np.uint8))

        template = Image.open(os.path.join(os.path.dirname(__file__), "apple.png"))
        # resize template
        ...
        for i in range(length):
            #background_color = Image.fromarray(np.random.normal(255, 5, (size[1], size[0], 3)).astype(np.uint8))
            frame_color = background_color.copy()

            x += vx
            y += vy

            # Check if the cow hits the edges and reverse its direction
            if x < 0 or x + template.size[0] > size[0]:
                vx = -vx
            if y < 0 or y + template.size[1] > size[1]:
                vy = -vy
            
            # Randomly resize the cow
