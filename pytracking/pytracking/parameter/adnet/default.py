...
def parameters():
    ...
    params.opts = {
        'imgSize' : [112, 112, 3],
        'train_dbs' : ['vot15', 'vot14', 'vot13', "got10k"],
        'test_db' : 'otb',
        'train': {
            'weightDecay' : 0.0005,
            'momentum' : 0.9,
    ...
    params.batch_frames = 3 
    params.n_pos_train = int(180 * 0.8)
    params.n_neg_train = int(40 * 0.8)
    ...
    params.rl_sequence_length = 40 #40
    params.n_batches_rl = 1  #1 Batch this many simulations before optimizing
    params.rl_gamma = 0.1  #0.5 \gamma for RL RETURNS reward signal
    ...
    params.n_heads = 5
    params.heads_checkpoint_path = Path(env.network_path) / "adnet_checkpoints" / "ensemble"

    ## Use EfficientNet backbone instead of VGG-M
